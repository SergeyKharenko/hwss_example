...
static void hwss_hnet_w5500_check_state_timer_cb(void *args){
    ...
    if(ir&W5500_IR_CONFLICT){        
        if(esp_event_post_to(hnet_w5500->super.elp_hdl,HWSS_HNET_EVENT,HWSS_HNET_EVENT_IP_CONFLICT,NULL,0,0)!=ESP_OK){
            ESP_LOGE(TAG,"fail to post event");
        }
    }

    if(ir&W5500_IR_UNREACH){        
        if(esp_event_post_to(hnet_w5500->super.elp_hdl,HWSS_HNET_EVENT,HWSS_HNET_EVENT_DEST_UNREACH,NULL,0,0)!=ESP_OK){
            ESP_LOGE(TAG,"fail to post event");
        }
    }

    if(ir&W5500_IR_PPPoE){        
        if(esp_event_post_to(hnet_w5500->super.elp_hdl,HWSS_HNET_EVENT,HWSS_HNET_EVENT_PPPOE_CLOSE,NULL,0,0)!=ESP_OK){
            ESP_LOGE(TAG,"fail to post event");
        }
    }

    if(ir&W5500_IR_MP){        
        if(esp_event_post_to(hnet_w5500->super.elp_hdl,HWSS_HNET_EVENT,HWSS_HNET_EVENT_MAGIC_PACK,NULL,0,0)!=ESP_OK){
            ESP_LOGE(TAG,"fail to post event");
        }
    }
