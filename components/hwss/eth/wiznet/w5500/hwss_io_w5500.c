...
static esp_err_t hwss_io_w5500_spi_read(hwss_io_t *io, uint32_t cmd, uint32_t addr, uint8_t *data){
    return hwss_io_spi_cache_read(io,0,addr | (_W5500_SPI_READ_ | _W5500_SPI_VDM_OP_),data,1);
}

static esp_err_t hwss_io_w5500_spi_write(hwss_io_t *io, uint32_t cmd, uint32_t addr, const uint8_t *data){
    return hwss_io_spi_cache_write(io,0,addr | (_W5500_SPI_WRITE_ | _W5500_SPI_VDM_OP_),data,1);
}

static esp_err_t hwss_io_w5500_spi_read_buf(hwss_io_t *io, uint32_t cmd, uint32_t addr, uint8_t *data, size_t data_len){
    return hwss_io_spi_cache_read(io,0,addr | (_W5500_SPI_READ_ | _W5500_SPI_VDM_OP_),data,data_len);
}

static esp_err_t hwss_io_w5500_spi_write_buf(hwss_io_t *io, uint32_t cmd, uint32_t addr, const uint8_t *data, size_t data_len){
    return hwss_io_spi_cache_write(io,0,addr | (_W5500_SPI_WRITE_ | _W5500_SPI_VDM_OP_),data,data_len);
}

static esp_err_t hwss_io_w5500_spi_read_mem(hwss_io_t *io, uint32_t cmd, uint32_t addr, uint8_t *data, size_t data_len){
    return hwss_io_spi_direct_read(io,0,addr | (_W5500_SPI_READ_ | _W5500_SPI_VDM_OP_),data,data_len);
}

static esp_err_t hwss_io_w5500_spi_write_mem(hwss_io_t *io, uint32_t cmd, uint32_t addr, const uint8_t *data, size_t data_len){
    return hwss_io_spi_direct_write(io,0,addr | (_W5500_SPI_WRITE_ | _W5500_SPI_VDM_OP_),data,data_len);
}

static hwss_io_t *hwss_io_new_w5500_spi(const hwss_io_spi_config_t* config){
    hwss_io_t *ret=NULL;
    hwss_io_spi_t *io_w5500_spi=NULL;

    io_w5500_spi=hwss_io_new_spi(config);
    ...
    io_w5500_spi->super.read_mem=hwss_io_w5500_spi_read_mem;
    io_w5500_spi->super.write_mem=hwss_io_w5500_spi_write_mem;
