...
IRAM_ATTR static void hwss_hir_isr_handler(void *args){
    ...
    esp_event_isr_post_to(hir->elp_hdl,HWSS_HIR_EVENT,HWSS_HIR_EVENT_TRIGGER,NULL,0,&unblock);
}
...
static esp_err_t hwss_hir_register_handler(hwss_hir_t *hir, esp_event_handler_t handler, void *args){
    return esp_event_handler_register_with(hir->elp_hdl,HWSS_HIR_EVENT,HWSS_HIR_EVENT_TRIGGER,handler,args);
}

static esp_err_t hwss_hir_unregister_handler(hwss_hir_t *hir, esp_event_handler_t handler){
    return esp_event_handler_unregister_with(hir->elp_hdl,HWSS_HIR_EVENT,HWSS_HIR_EVENT_TRIGGER,handler);
}
