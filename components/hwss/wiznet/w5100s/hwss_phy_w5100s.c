...
static void hwss_phy_w5100s_check_timer_cb(void *args){
    ...
    if(link!=link_p){
        if(link==HWSS_LINK_UP){
            if(hwss_event_post(HWSS_EVENT,HWSS_EVENT_CONNECTED,(void *)&phy_w5100s->super,sizeof(hwss_phy_t *),0)!= ESP_OK)
                ESP_LOGE(TAG,"fail to post event");
            ESP_LOGD(TAG,"Connected");
        }
        else{
            if(hwss_event_post(HWSS_EVENT,HWSS_EVENT_DISCONNECTED,(void *)&phy_w5100s->super,sizeof(hwss_phy_t *),0)!=ESP_OK)
                ESP_LOGE(TAG,"fail to post event");
...
esp_err_t hwss_phy_w5100s_reset(hwss_phy_t *phy){
...
esp_err_t hwss_phy_w5100s_init(hwss_phy_t *phy){
    esp_err_t ret = ESP_OK;
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
    
    atomic_store(&(phy_w5100s->link),HWSS_LINK_DOWN);

    ESP_GOTO_ON_ERROR(esp_timer_start_periodic(phy_w5100s->check_timer, phy_w5100s->check_period_ms*1000),
                        err,TAG,"cannot start timer");
err:
    return ret;
}

esp_err_t hwss_phy_w5100s_deinit(hwss_phy_t *phy){
    esp_err_t ret = ESP_OK;
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
    ESP_GOTO_ON_ERROR(esp_timer_stop(phy_w5100s->check_timer),err,TAG,"cannot stop timer");

err:
    return ret;
}

esp_err_t hwss_phy_w5100s_autonego_ctrl(hwss_phy_t *phy, hwss_phy_autoneg_cmd_t cmd, bool *autonego_en_stat){
    esp_err_t ret = ESP_OK;
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
    uint8_t stat;
    uint8_t mode;
...
esp_err_t hwss_phy_w5100s_set_link(hwss_phy_t *phy, hwss_link_t link){
    esp_err_t ret=ESP_OK;
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
    ...
    if(link!=link_p){
        atomic_store(&(phy_w5100s->link),link);
        if(link==HWSS_LINK_UP)
            ESP_GOTO_ON_ERROR(esp_event_post(HWSS_EVENT,HWSS_EVENT_CONNECTED,(void *)&phy_w5100s->super,sizeof(hwss_phy_t),0),
                            err,TAG,"fail to post event");
        else
            ESP_GOTO_ON_ERROR(esp_event_post(HWSS_EVENT,HWSS_EVENT_DISCONNECTED,(void *)&phy_w5100s->super,sizeof(hwss_phy_t),0),
                            err,TAG,"fail to post event");
    }
...
esp_err_t hwss_phy_w5100s_get_link(hwss_phy_t *phy, hwss_link_t *link){
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
...
esp_err_t hwss_phy_w5100s_get_speed(hwss_phy_t *phy, hwss_speed_t *speed){
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
...
esp_err_t hwss_phy_w5100s_get_duplex(hwss_phy_t *phy, hwss_duplex_t *duplex){
    hwss_phy_w5100s_t *phy_w5100s=__containerof(phy,hwss_phy_w5100s_t,super);
