...
static esp_err_t hwss_io_w5500_spi_read(hwss_io_t *io, uint32_t cmd, uint32_t addr, uint8_t *data){
    ...
    if(IO_W5500_SPI_LOCK(io_w5500_spi)){
        if(spi_device_polling_transmit(io_w5500_spi->hdl,&trans)!=ESP_OK){
            ESP_LOGE(TAG, "%s(%d): read reg failed", __FUNCTION__, __LINE__);
            ret =  ESP_FAIL;
        }
        IO_W5500_SPI_UNLOCK(io_w5500_spi);
    }
    else{
        ret = ESP_ERR_TIMEOUT;
    }
...
static hwss_io_t *hwss_io_new_w5500_spi(hwss_io_spi_config_t* config){
    ...
    config->spi_devcfg.mode=0;
    memcpy(&io_w5500_spi->cfg,config,sizeof(hwss_io_spi_config_t));
    ...
    io_w5500_spi->lock=xSemaphoreCreateMutex();
